...
def auto_cast_bool(target_dtype="int32"):
    """
    Decorator that automatically casts boolean inputs to a target dtype and casts
    results back to boolean if needed. This is used for operations like scatter and
    gather that don't natively support boolean types in CoreML.
    
    Args:
        target_dtype: The dtype to cast boolean inputs to (default: "int32")
    
    Returns:
        A decorator function that wraps scatter/gather operations
    """
    def decorator(func):
        @wraps(func)
